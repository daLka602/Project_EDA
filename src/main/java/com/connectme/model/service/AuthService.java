...
public class AuthService {
    ...
    public boolean login(String username, String plainPassword) {
        if (!isValidUsername(username) || !isValidPassword(plainPassword)) {
    ...
    /**
     * NOVO: Verificar se usuário existe
     */
    ...
    public boolean unlockUser(String username) {
        if (userDAO instanceof UserDAO) {
            return ((UserDAO) userDAO).unlockUser(username);
        }
        return false;
    }
    ...
    // Switch tradicional (funciona em todas versões)
    switch (score) {
        case 0:
        case 1:
        case 2:
